"""
step_7.15_phb_pathway_flexibility.py
...
(reusing step_7.12's pathway-distance ranking) and plots each reaction's
...
Outputs:
    step_7_outputs/step_7.15_pathway_flexibility.csv
    step_7_outputs/step_7.15_pathway_flexibility_bar_chart.png
"""
...
MAX_REACTIONS_TO_PLOT = 150   # hard cap — see step7_utils.capped_figsize for why this matters
...
def main():
    ...
    scoped_ids = cap_item_list(scoped["reaction_id"].tolist(), MAX_REACTIONS_TO_PLOT, label="pathway reactions")
    ...
    csv_path = os.path.join(OUTPUT_DIR, "step_7.15_pathway_flexibility.csv")
    merged.to_csv(csv_path, index=False)
    print(f"Flexibility table written to: {csv_path}")

    # --- Bar chart, ordered by pathway distance, colored by distance --------
    fig_width = capped_figsize(len(merged), per_item=0.2, min_size=10.0, max_size=24.0)
    ...
    png_path = os.path.join(OUTPUT_DIR, "step_7.15_pathway_flexibility_bar_chart.png")
