...
these are chosen to probe exactly that behavior):
  ...
  1. "standard"   — batch growth, nitrogen never limiting. Biomass
                     ...
                     (maltose) is exhausted; PHB stays low.
  2. "n_limited"   — nitrogen supplied at a low initial concentration,
                     so it runs out mid-simulation while maltose is
...
def run_dfba(model, biomass, phb, carbon, nitrogen, oxygen, scenario):
    ...
    for step in range(n_steps + 1):
        t = step * DT_H
        records.append({
            "time_h": t, "biomass_gL": X, "maltose_mM": mal,
        ...
        with model as m:
            ...
            carbon_rxn_m.lower_bound = -v_mal
        ...
        if not status_ok or sol.status != "optimal":
            ...
            v_mal_actual = -sol.fluxes[carbon.id]
...
def main():
    ...
    carbon = model.reactions.get_by_id(MALTOSE_EXCHANGE_ID)
    ...
    for scenario in scenarios:
        ...
        results[scenario] = df
        csv_path = os.path.join(OUTPUT_DIR, f"step_7.05_dfba_{scenario}__maltose.csv")
    ...
    panels = [
        ...
        ("maltose_mM", "Maltose (mM)", axes[0, 1]),
    ...
    png_path = os.path.join(OUTPUT_DIR, "step_7.05_dfba_timecourse_maltose.png")
