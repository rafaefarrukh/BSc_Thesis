...
of reactions in the PHB pathway neighborhood (from step_7.11) changes as
the fraction is relaxed from 100% down to 50%.

By default the FVA scope is restricted to the PHB pathway neighborhood
(reusing step_7.11's distance ranking) to keep runtime reasonable — set
...
Requires step_7.11 to have been run first if REACTION_SCOPE == "pathway".

Outputs:
    step_7_outputs/step_7.15_fva_frac<pct>_<loopless|standard>.csv   (x6)
    step_7_outputs/step_7.15_fva_summary_stats.csv
    step_7_outputs/step_7.15_pathway_ranges_across_fractions.png
"""
...
# synthesis (reuses step_7.11's output); "all" = every reaction in the model
REACTION_SCOPE = "pathway"
PATHWAY_MAX_DISTANCE = 6
MAX_REACTIONS_IN_SCOPE = 150
...
def get_reaction_scope(model):
    if REACTION_SCOPE == "all":
        ...
        if len(ids) > MAX_REACTIONS_IN_SCOPE:
            print(f"  (Capping reactions from {len(ids)} to {MAX_REACTIONS_IN_SCOPE})")
            ids = ids[:MAX_REACTIONS_IN_SCOPE]
        return ids, None

    if not os.path.exists(DISTANCE_CSV):
        print(f"WARNING: {DISTANCE_CSV} not found (run step_7.11 first). "
    ...
    if len(raw_ids) > MAX_REACTIONS_IN_SCOPE:
        print(f"  (Capping pathway reactions from {len(raw_ids)} to {MAX_REACTIONS_IN_SCOPE})")
        ids = raw_ids[:MAX_REACTIONS_IN_SCOPE]
    else:
        ids = raw_ids
...
def main():
    ...
    for fraction in FRACTIONS:
        for loopless in (False, True):
            ...
            csv_path = os.path.join(OUTPUT_DIR, f"step_7.15_fva_{label}.csv")
            fva.to_csv(csv_path)
    ...
    summary_csv = os.path.join(OUTPUT_DIR, "step_7.15_fva_summary_stats.csv")
    ...
    if dist_df is not None:
        # Compute dynamic figsize inside clean maximum bounds matching requirements
        fig_width = max(10.0, min(24.0, len(reaction_ids) * 0.2))
        ...
        png_path = os.path.join(OUTPUT_DIR, "step_7.15_pathway_ranges_across_fractions.png")
        ...
        print("\n(Skipping pathway-ranges figure: no pathway-distance scoping available. "
              "Run step_7.11 first and set REACTION_SCOPE='pathway' to get this plot.)")
